...
export default function EmailNotify() {
  ...
  useEffect(() => {
    // console.log("Inside another use Effect", data, data.type, data.length)

    const finalGroupData = [];
    const patient_id_unique_list = [];
    
    const mappedGroupData = data.map((item) => {
      
      let newData = data.filter((value) => value.Patient_id === item.Patient_id);

      // console.log("newData: ", newData)

      if (patient_id_unique_list.indexOf(newData[0].Patient_id) === -1) {
        patient_id_unique_list.push(newData[0].Patient_id);
        finalGroupData.push(newData)
      }

      return item;
    })

    console.log("final Grouped Data: ",finalGroupData, finalGroupData[0], finalGroupData[1])

    setgroupedData(finalGroupData)
  }, [data])
  ...
  return (
    <div>
      <p style={{ fontSize: '30px', textAlign: 'center', color: 'indigo' }}><strong>Patient Notifications Panel</strong></p>
      <Paper>
                ...
                .map((row, index) => {
                  ...
                  for (var i = 0; i<row.length; i++) {
                    if (i == 0){
                      retRow.push(
                        <StyledTableRow>
                          <StyledTableCell align="left">{row[i].Patient_name}</StyledTableCell>
                          <StyledTableCell align="left">{row[i].Guardian_Email}</StyledTableCell>
                          <StyledTableCell align="left">{row[i].Practitioner}</StyledTableCell>
                          <StyledTableCell>{riskscore(row[i].Risk_Category)}</StyledTableCell>
                          <StyledTableCell key={index}> <button key={index} type="button" class="btn btn-primary" onClick={() => sendemail(row[i].Patient_name, row[i].Practitioner, row[i].Guardian_Email)}>Send &nbsp;<TelegramIcon /></button></StyledTableCell>
                        </StyledTableRow>
                      )
                    } else {
                      retRow.push(
